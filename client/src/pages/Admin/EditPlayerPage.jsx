...
import { useGetAllUsersQuery, useUpdatePlayerMutation } from "@/features/admin/adminApi";
import { useTranslation } from "react-i18next";

export default function EditPlayerPage({ id, onBack }) {
    const { t } = useTranslation();
    const { data: users, isLoading } = useGetAllUsersQuery();
    const [updatePlayer, { isLoading: isUpdating, isSuccess, error }] = useUpdatePlayerMutation();
    ...
    return (
        <div className="w-full h-full flex flex-col items-stretch justify-center">
            ...
            <div className="text-2xl font-bold mb-8 text-[var(--color-primary)]">{t("editPlayerTitle")}</div>
            <form
            ...
            >
                ...
                */}
                ...
                {isSuccess && (
                    <div className="text-green-600 mt-1">{t("playerUpdated")}</div>
                )}
                {error && (
                    <div className="text-red-600 mt-1">
                        {t("errorUpdatingPlayer")} {error?.error || ""}
                    </div>
                )}
            </form>
        </div>
