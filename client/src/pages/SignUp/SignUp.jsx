...
export default function SignUp() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const initialTheme = "default"; // Начальная тема для нового пользователя
      dispatch(
          setUser({
            nickname: formData.nickname,
            points: 0,
            ...
            firstname: formData.firstname,
            lastname: formData.lastname,
            currentTheme: initialTheme, // Добавляем начальную тему
          })
      );
      dispatch(setToken(data.token)); // Сохраняем токен
      dispatch(setTheme(initialTheme)); // Устанавливаем тему
  ...
  return (
      <div className="min-h-screen flex items-center justify-center bg-gray-100">
        <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md sm:max-w-sm">
          ...
          {error && <p className="text-red-500 text-center mb-4">{error.message || "Registration failed"}</p>}
